...
# 本地真实 AI_DIRECT 验收专用启动脚本。
# 不在脚本内写入 API Key；启动前请在当前 PowerShell 会话设置 OPENAI_API_KEY 或 AI_API_KEY。

$ErrorActionPreference = 'Stop'
$runDir = Join-Path $ProjectRoot '.run'
$frontendRoot = Join-Path $ProjectRoot 'frontend'

function Get-ConfiguredAiApiKey {
    foreach ($name in @('OPENAI_API_KEY', 'AI_API_KEY')) {
        ...
        if ($value -and $value.Trim() -ne '' -and $value.Trim().ToLower() -ne 'please_change_me') {
            return $value.Trim()
        }
    }
    return $null
...
function Start-BackendAi() {
    ...
    $command = "${deepseekKeyForward}set CODEFORGE_FORCE_RULE_ONLY=false && set SERVER_PORT=$BackendPort && .\mvnw.cmd -Dspring-boot.run.main-class=com.codeforge.ai.CodeForgeAiApplication -Dspring-boot.run.profiles=local -Dspring-boot.run.arguments=--server.servlet.context-path=/api spring-boot:start 1>`"$stdout`" 2>`"$stderr`""
    ...
    $listener = Wait-ForListener -port $BackendPort -timeoutSeconds 30
    if (-not $listener) {
    ...
    Write-PortFile 'backend.port' $BackendPort
    Write-Host "Backend (AI dev profile) is listening on $BackendPort. PID=$($listener.OwningProcess)" -ForegroundColor Green
}
...
function Start-Frontend() {
    ...
    $command = "/c set ""VITE_API_BASE_URL=$apiBaseUrl"" && set ""VITE_APP_BASE_URL=$appBaseUrl"" && npm.cmd run dev -- --host 127.0.0.1 --port $FrontendPort"
...
if (-not (Get-ConfiguredAiApiKey)) {
    Write-Host "ERROR: 未检测到有效的 OPENAI_API_KEY 或 AI_API_KEY。" -ForegroundColor Red
    Write-Host "请先在当前 PowerShell 会话设置环境变量，例如：" -ForegroundColor Yellow
    Write-Host '  $env:OPENAI_API_KEY="your-key"' -ForegroundColor Yellow
    Write-Host '  $env:CODEFORGE_FORCE_RULE_ONLY="false"' -ForegroundColor Yellow
    Write-Host "脚本不会读取或写入 git 跟踪文件中的密钥。" -ForegroundColor Yellow
    exit 1
}
...
Write-Host "=== Dev Process Start (AI_DIRECT) ===" -ForegroundColor Cyan
Write-Host "ProjectRoot: $ProjectRoot"
Write-Host "Spring profile: dev"
Write-Host "CODEFORGE_FORCE_RULE_ONLY: false"
Write-Host ""
$startFailed = $false
