...
class AiCompactRetryExhaustedTest {
    ...
    @Test
    void shouldFailWithAiOutputTruncatedWhenBothAttemptsAreLengthLimited() {
        given(invoker.streamWithAiProvidersOnly(any(), any(), any(), any(Integer.class), any()))
                .willReturn(lengthResult())
                .willReturn(lengthResult());
    ...
    private ModelChatResult lengthResult() {
        return ModelChatResult.success("{\"files\":[", "length", 100L, 8192L, 8292L, 100L, "deepseek", "deepseek-chat");
    }
