-- AI 零代码应用生成平台首版 DDL 草案
-- 目标：支撑首版 MVP 的身份、空间、应用、生成任务、版本、导出、配额、审计能力
-- 说明：
-- 1. 本文件为新架构草案，不覆盖现有 create_table.sql
-- 2. 默认数据库为 codeforge_ai
-- 3. 字符集统一使用 utf8mb4

CREATE DATABASE IF NOT EXISTS codeforge_ai
...
CREATE TABLE IF NOT EXISTS generation_task (
    ...
    CONSTRAINT uk_generation_task_scope_idem_key UNIQUE (workspace_id, app_id, idempotency_key),
...
CREATE TABLE IF NOT EXISTS generation_task_event (
    ...
    INDEX idx_generation_task_event_request_id (request_id)
...
CREATE TABLE IF NOT EXISTS generated_file (
    ...
    INDEX idx_generated_file_version_path (app_version_id, file_path(255)),
