...
默认只初始化已经存在的空数据库。若目标数据库不存在，脚本返回 `TARGET_DATABASE_DOES_NOT_EXIST`；只有显式传入 `-CreateDatabase` 且 `.env.local` 同时提供 `DB_ADMIN_USERNAME` 与 `DB_ADMIN_PASSWORD` 时，脚本才会使用管理员凭据创建数据库并授予最小权限，随后仍使用 `DB_USERNAME` / `DB_PASSWORD` 运行 Flyway。
